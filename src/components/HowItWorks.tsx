...
export default function HowItWorks() {
  return (
    <div className="w-[90%] sm:w-[85%] md:w-[100%] lg:max-w-[1300px] md:max-w-[900px] mx-auto mt-10 space-y-5 md:space-y-7 md:p-6 lg:p-12">
      <h1 className="text-xl sm:text-2xl md:text-4xl font-medium leading-[1.3] md:leading-[1.35] text-center text-[#1F1F1F]">
        How To Summarize Podcast
      </h1>

      <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 md:gap-10 justify-items-center">
        {data.map((step) => (
    ...
    <div className="w-full px-5 py-10 flex flex-col justify-between space-y-3 border border-[#CBD5E1] rounded-2xl hover:-translate-y-1 hover:shadow-lg transition ease-in-out delay-150 duration-300 mb-10">
      <div>
        <div className="flex items-center gap-2 mb-2">
          <span className="rounded-lg bg-[#F97316] text-white text-center flex items-center justify-center h-8 w-8">
            {step}
          </span>
          <h3 className="text-base md:text-xl font-semibold text-[#1A1A1A]">
            {title}
          </h3>
        </div>
        <p className="text-sm md:text-base text-[#555555]">{description}</p>
      </div>
      <Image
        src={src}
        alt={title}
